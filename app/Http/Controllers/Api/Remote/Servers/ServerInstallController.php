...
namespace Hyperodactyl\Http\Controllers\Api\Remote\Servers;
...
use Hyperodactyl\Exceptions\Http\HttpForbiddenException;
use Hyperodactyl\Repositories\Eloquent\ServerRepository;
use Hyperodactyl\Events\Server\Installed as ServerInstalled;
use Illuminate\Contracts\Events\Dispatcher as EventDispatcher;
use Hyperodactyl\Http\Requests\Api\Remote\InstallationDataRequest;
...
class ServerInstallController extends Controller
{
    ...
    public function store(InstallationDataRequest $request, string $uuid): JsonResponse
    {
        ...
        if ($isInitialInstall && config()->get('hyperodactyl.email.send_install_notification', true)) {
            $this->eventDispatcher->dispatch(new ServerInstalled($server));
        } elseif (!$isInitialInstall && config()->get('hyperodactyl.email.send_reinstall_notification', true)) {
            $this->eventDispatcher->dispatch(new ServerInstalled($server));
        }
