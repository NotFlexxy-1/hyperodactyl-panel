...
namespace Hyperodactyl\Console\Commands\Environment;

use Illuminate\Console\Command;
use Illuminate\Contracts\Console\Kernel;
use Hyperodactyl\Traits\Commands\EnvironmentWriterTrait;
...
class AppSettingsCommand extends Command
{
     ...
     *
     * @throws \Hyperodactyl\Exceptions\HyperodactylException
     */
    public function handle(): int
    {
        ...
        $this->variables['APP_SERVICE_AUTHOR'] = $this->option('author') ?? $this->ask(
            'Egg Author Email',
            config('hyperodactyl.service.author', 'unknown@unknown.com')
        );
        ...
        $this->output->comment('Please reference https://hyperodactyl.io/panel/1.0/additional_configuration.html#telemetry for more detailed information regarding telemetry data and collection.');
        $this->variables['HYPERODACTYL_TELEMETRY_ENABLED'] = $this->option('telemetry') ?? $this->confirm(
            'Enable sending anonymous telemetry data?',
            config('hyperodactyl.telemetry.enabled', true)
        ) ? 'true' : 'false';
