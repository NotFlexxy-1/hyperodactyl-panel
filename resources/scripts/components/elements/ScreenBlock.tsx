...
const ActionButton = styled(Button)`
    ${tw`rounded-full w-9 h-9 flex items-center justify-center p-0`};
...
const ScreenBlock = ({ title, image, message, onBack, onRetry }: ScreenBlockProps) => (
    <PageContentBlock>
        <div css={tw`flex justify-center`}>
            <div
                css={tw`w-full sm:w-3/4 md:w-1/2 p-12 md:p-20 rounded-3xl text-center relative border animate-scale-in`}
                style={{
            ...
            >
                ...
                <img src={image} css={tw`w-2/3 h-auto select-none mx-auto opacity-90`} />
                <h2 css={tw`mt-10 font-header font-bold text-3xl`} style={{ color: 'rgb(var(--hyper-text))' }}>
                    {title}
                </h2>
                <p css={tw`text-sm mt-2`} style={{ color: 'rgb(var(--hyper-text-muted))' }}>
                    {message}
                </p>
            </div>
        </div>
